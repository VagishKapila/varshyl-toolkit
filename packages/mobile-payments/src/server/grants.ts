...
import { mapGrant, mapPromo } from './grants-mappers.js';
...
): Promise<ReturnType<typeof mapGrant>> {
  const { rows } = await db.query(
    `INSERT INTO mp_granted_access (user_id, product_slug, granted_by, reason, expires_at)
     ...
     ON CONFLICT (user_id, product_slug) DO UPDATE SET
       ...
       created_at = NOW()
     RETURNING *`,
    [input.userId, input.productSlug, input.grantedBy, input.reason ?? null, input.expiresAt ?? null],
  );
  return mapGrant(rows[0] as Record<string, unknown>);
}

/** Super admin: revoke a grant */
export async function revokeAccess(db: Db, userId: string, productSlug: string): Promise<boolean> {
...
export async function listGrants(db: Db, productSlug: string) {
  const { rows } = await db.query(
...
export async function listPromoCodes(db: Db, productSlug: string) {
  const { rows } = await db.query(
...
export async function redeemPromoCode(
  ...
  try {
    ...
    if (maxUses != null && Number(promo.uses ?? 0) >= maxUses) {
      await client.query('ROLLBACK');
    ...
    await client.query(`INSERT INTO mp_promo_redemptions (code_id, user_id) VALUES ($1, $2)`, [promo.id, userId]);
    await client.query(`UPDATE mp_promo_codes SET uses = uses + 1 WHERE id = $1`, [promo.id]);
    const permanent = Boolean(promo.grants_permanent);
    const days = promo.grants_days != null ? Number(promo.grants_days) : null;
    const expiresAt = permanent || days == null ? null : new Date(Date.now() + days * 86400000);
    await client.query(
      `INSERT INTO mp_granted_access (user_id, product_slug, granted_by, reason, expires_at)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (user_id, product_slug) DO UPDATE SET
         granted_by = EXCLUDED.granted_by, reason = EXCLUDED.reason,
         expires_at = EXCLUDED.expires_at, revoked_at = NULL, created_at = NOW()`,

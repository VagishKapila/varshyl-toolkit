...
describe('bundled dist (tsup) - no fs migration IO', () => {
  it('inlines SQL into the bundle (readFileSync absent)', () => {
    ...
    expect(bundleSource).toMatch(/CREATE TABLE IF NOT EXISTS mp_schema_migrations/);
  });
});
...
describeWithPostgres('bundled dist (tsup) - migrations + self-test', () => {
  const bundleDir = mkdtempSync(join(tmpdir(), 'mp-bundled-'));
  ...
  beforeAll(async () => {
    ...
    expect(bundleSource).toMatch(/mp_schema_migrations/);
  ...
  it('runs inlined migrations and verifies mp_subscriptions table', async () => {
    const bundleUrl = pathToFileURL(join(bundleDir, 'index.js')).href;
    const mod = await import(bundleUrl);

    expect(mod.runMigrations).toBeTypeOf('function');
    expect(mod.mpSelfTest).toBeTypeOf('function');

...
describe('bundled dist (tsup) — no fs migration IO', () => {
  it('inlines SQL into the bundle (readFileSync absent)', () => {
    ...
    expect(bundleSource).toMatch(/CREATE TABLE IF NOT EXISTS oce_schema_migrations/);
  });
});

const describeWithPostgres = isDockerAvailable() ? describe : describe.skip;

describeWithPostgres('bundled dist (tsup) — migrations + consent flow', () => {
  const bundleDir = mkdtempSync(join(tmpdir(), 'oce-bundled-'));
  ...
  beforeAll(async () => {
    ...
    expect(bundleSource).toMatch(/oce_schema_migrations/);
  ...
  it('runs inlined migrations, seeds consents, records and reads consent', async () => {
    const bundleUrl = pathToFileURL(join(bundleDir, 'index.js')).href;
    const mod = await import(bundleUrl);

    expect(mod.runMigrations).toBeTypeOf('function');
    expect(mod.oceSelfTest).toBeTypeOf('function');

    const selfTest = await mod.oceSelfTest({ pool, productName: 'BundledProduct' });
    expect(selfTest.migrationsOk).toBe(true);
    expect(selfTest.seedOk).toBe(true);
    expect(selfTest.consentKeysFound).toEqual(
      expect.arrayContaining(['terms_of_service', 'privacy_policy', 'ai_training']),
    );

    const consent = mod.createConsentModule({ pool });
    const userId = `bundled-${Date.now()}`;
    ...
    expect(await consent.hasUserConsented(userId, 'terms_of_service')).toBe(true);
  }, 30_000);
});
